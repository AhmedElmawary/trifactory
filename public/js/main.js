        ...
        success: function (result) {
            if (result) {
                document
                    ...
                    .setCustomValidity(
                        "This phone is linked to a different email address"
                    );
            } else {
                document
                    .getElementsByName("ticket_1_phone")[0]
...
function onFileChange() {
    if ($("#national_id_image").val()) {
        ...
        if (
            ...
            // 15728640
            document
                .getElementById("national_id_image")
                .setCustomValidity("Input file must be not greater than 25MB");
        } else {
            ...
            if (!switching) {
                ...
                for (let i = 0; i < b.length - 1; i++) {
                    if (b[i].innerHTML.toLowerCase().search("other") != -1) {
                        b[b.length - 1].parentNode.insertBefore(
                            b[i],
                            b[b.length - 1].nextSibling
                        );
                    }
        ...
        and mark the switch as done: */
            ...
            if (!switching) {
                ...
                for (let i = 0; i < b.length - 1; i++) {
                    ...
                    if (
                        b[i].innerHTML.toLowerCase().search("team name") != -1
                    ) {
                        b[0].parentNode.insertBefore(b[i], b[0]);
                    }
                }
        ...
        $(".leaderboard_year").on("change", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    let maleContent =
                        "<thead><tr><th scope='col'>Rank</th><th scope='col'>Name</th><th scope='col'>Nationality</th>" +
                        "<th scope='col'>Age Group</th><th scope='col'>Club</th><th scope='col'>Points</th></tr></thead>";
                    let femaleContent =
                        "<thead><tr><th scope='col'>Rank</th><th scope='col'>Name</th><th scope='col'>Nationality</th>" +
                        "<th scope='col'>Age Group</th><th scope='col'>Club</th><th scope='col'>Points</th></tr></thead>";
                    ...
                    if (clubData.length) {
                        $("#viewMoreClub").show();
                        $("#clubBody").append(clubContent);
                        $("#rankClubUrl").attr(
                            "href",
                            "/leaderboard/" + year + "#pills-rankings-club"
                        );
                        ...
                        // $("#rankClubUrl").attr(
                        //     "href",
                        //     "/leaderboard#pills-rankings-club"
                        // );
                    }
                    ...
                    if (maleData.length) {
                        $("#viewMoreMale").show();
                        $("#maleBody").append(maleContent);
                        $("#rankMalesUrl").attr(
                            "href",
                            "/leaderboard/" + year + "#pills-rankings-male"
                        );
                        $("#rankMalesUrl").text("View More");
                    } else {
                        $("#maleBody").append(
                            "<h4 align='center'>No Data found for Year: " +
                                year +
                        ...
                        // $("#rankMaleUrl").attr(
                        //     "href",
                        //     "/leaderboard#pills-rankings-male"
                        // );
                    }
                    ...
                    if (femaleData.length) {
                        $("#viewMoreFemale").show();
                        $("#femaleBody").append(femaleContent);
                        $("#rankFemalesUrl").attr(
                            "href",
                            "/leaderboard/" + year + "#pills-rankings-female"
                        );
                        $("#rankFemalesUrl").text("View More");
                    } else {
                        $("#femaleBody").append(
                            "<h4 align='center'>No Data found for Year: " +
                                year +
    ...
    function initEventDetails() {
                    ...
                    success: function (user) {
                        ...
                        if (user["club"] != "") {
                            ...
                            if (val) {
                                ...
                                $("#club").val(
                                    $("#club option:contains(Other)").val()
                                );
        ...
        $("#ticket_1_use_someone").on("change", function () {
            ...
            if ($("#club").length) {
                ...
                document.getElementById("club").style.backgroundColor =
                    "#f5f5f5";
                if ($("#others").length) {
                    document.getElementById(
                        "others"
                    ...
                    if (data.length) {
                        ...
                        $.each(questions, function (key, question) {
                            ...
                            if (question.answertype.type === "file") {
                                ...
                                str += " />";
                                str +=
                                    ' <span class="fas fa-upload"></span> &nbsp;&nbsp;Upload valid copy of ID/Passport (Less than 25MB ) <span style="float:right;color:green; display:none;" class="far fa-check-circle"></span><span style="float:right;color:red;" class="far fa-times-circle"></span></label>';
                            }
