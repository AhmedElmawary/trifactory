...
<section class="container no-height no-separator">
    <h3 class="section-title">
        <img src="/images/payment-icon.svg" alt="payment-icon" class="icon" />Payment
    </h3>
    ...
    <div class="row col-lg-6">
        <div class="input-group">
            @csrf
            <input id="vode" name="code" value="@if($voucher) {{$voucher->getAttributes()['code']}} @endif" type="text" class="form-control dark-bg" placeholder="Type the Code Yala" @if($voucher) disabled @endif />
        </div>
    </div>
    ...
    <div class="cart-details-container">
        ...
        <div class="cart-summary-item separator">
            Promocode: {{ $item['conditions'][0]->getAttributes()['code'] }}
            <span class="float-right"> EGP
                @if(count($item['conditions']))
    ...
    <div class="cart-payment-container">
        <form method="POST" action="{{ url('/buy-tickets') }}">
            ...
            <div class="form-check" style="display: none;">
                <input class="form-check-input" type="radio" id="paymet_method_card" name="paymet_method" value="card" checked />
                <label class="form-check-label" for="paymet_method_card">Credit Card</label>
            </div>
